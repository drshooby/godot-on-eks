...
> **Status (see [`infra/02-uat/README.md`](../infra/02-uat/README.md) for current state):** the Gateway API / Envoy Gateway + cert-manager path below was **superseded by issue #16** in favor of a **Terraform-managed ALB + ACM cert + Kong `Ingress` controller**. The destroy-ordering rationale and open decisions are kept for history; treat sections referencing `GatewayClass` / `HTTPRoute` / cert-manager as design notes, not the shipped implementation.
